...
function HiddenSize({ value }) {
  return <input type="hidden" name="size" value={value} />;
}
...
const Enquire = forwardRef(function Enquire(props, ref) {
  ...
  const handleNameChange = useCallback((e) => setForm(f => ({ ...f, name: e.target.value })), []);
  const handleEmailChange = useCallback((e) => setForm(f => ({ ...f, email: e.target.value })), []);
  const handleCompanyChange = useCallback((e) => setForm(f => ({ ...f, company: e.target.value })), []);
  const handleDatesChange = useCallback((e) => setForm(f => ({ ...f, dates: e.target.value })), []);
  const handleMessageChange = useCallback((e) => setForm(f => ({ ...f, message: e.target.value })), []);
  ...
  return (
    <section id="enquire" data-screen-label="07 Enquire" style={{
      ...
      <div className="grid-2" style={{
        ...
        padding: '140px 48px 100px', alignItems: 'start',
      }}>
        <div>
          <Eyebrow>
          ...
          {submitted ? (
            ...
            >
              ...
              <Field id="f-size" label="Group Size">
                <HiddenSize value={form.size} />
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, paddingTop: 14 }}>
                  {sizes.map((sizeValue) => {
                    const active = form.size === sizeValue;
                    return (
                      <button
                        key={sizeValue}
                        type="button"
